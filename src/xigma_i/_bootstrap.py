"""sys.path wiring so ``xigma_i`` can import ``compton_io`` without it
being pip-installed.

``compton_io`` lives in a sibling checkout whose directory name and exact
path aren't a stable package-manager path (same situation
...
# .../xigma_i-repo/src/xigma_i -> .../xigma_i-repo -> .../ComptonSuite (sibling dir)
_SUITE_ROOT = _THIS_DIR.parents[2]

_COMPTON_IO_MARKER = "src/compton_io/constants.py"
_ENV_VAR = "XIGMA_COMPTON_IO_SRC"
...
def setup_paths() -> None:
    ...
    Resolution order, highest priority first:
      1. ``XIGMA_COMPTON_IO_SRC`` env var, if set -- used verbatim, no
    ...
    if override:
        ...
        if not matches:
            raise ImportError(
                f"xigma_i._bootstrap: could not find compton_io under "
