...
(gammaforge.io.bunch.sample_gaussian_bunch + particles.push_and_sample) and
...
def build_table_streaming(beam, laser, n_particles, n_steps, *, chunk_particles,
    ...
    at a time (gammaforge.io.bunch.sample_gaussian_bunch +
    ...
    beam: a gammaforge.io.bunch.GaussianElectronBeam (SI) describing the
        electron beam to sample -- electron sampling happens here via
        gammaforge.io, not internally to this model (see
        gammaforge.io.bunch.sample_gaussian_bunch); each chunk's SI
        MacroBunch is passed straight to particles.push_and_sample.
    laser: a gammaforge.io.laser.GaussianParaxialLaser describing the
    ...
    rng: passed to gammaforge.io.bunch.sample_gaussian_bunch for each
