...
``gammaforge.misc`` (the one shared cupy/numba backend-detection helper
...
``gammaforge.misc`` directly.

``me``/``c``/``el`` below come from ``gammaforge.io.units`` rather than
local literals -- the single shared source of truth also used by
``gammaforge.gui``/``kascade``. Only ``sigma_T`` (Thomson cross section,
...
from gammaforge.io.units import C_CM_S, EL_STATC, ME_G
from gammaforge.misc import detect_device as _detect_device  # noqa: F401 (re-exported for spectrum4d.py)
...
# considered "possibly inside the pulse" (gammaforge.io.bunch.
