...
from gammaforge.io import units as _io_units
from gammaforge.io.bunch import Bunch as _Bunch
from gammaforge.io.interaction import InteractionParameters as _InteractionParameters
from gammaforge.io.io_formats.sdds import save_elegant_ele as _save_elegant_ele
from gammaforge.io.laser import GaussianParaxialLaser as _GaussianParaxialLaser

# ---------------------------------------------------------------------------
# Physical constants (SI) -- from gammaforge.io.units (shared, pint-
# derived source of truth also used by gammaforge.gui/xigma_i) rather than
# hand-typed literals. Zero numeric change here: this module's previous
# literals already agreed with gammaforge.io's values to their quoted
...
class Config:
    ...
    (:mod:`gammaforge.io.interaction`) -- the single source of truth for
...
def run_simulation(cfg: Config, n_mc: int = 20_000, seed: int = 0,
    ...
    electrons : dict
        ...
        ``gammaforge.io.bunch.GaussianElectronBeam`` and draw one via
        ``gammaforge.io.bunch.sample_gaussian_bunch``, then convert the
