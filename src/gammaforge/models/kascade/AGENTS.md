...
project (Python package `gammaforge.gui`) via its
...
```python
from gammaforge.models.kascade import kascade as _kascade
from gammaforge.models.kascade.kascade_adapter import _bunch_to_kascade_electrons
...
arrays, converted from a `gammaforge.io.bunch.Bunch`) is required —
sampling the electron bunch is `gammaforge.io`'s job, not this module's; see
...
- Everything in `Config`/`Results` is **SI** (m, s, J, rad) — this is different from `xigma_i`'s CGS-like (cm, erg) convention. `gammaforge.gui`'s `model_api.py`/adapters were designed around this engine's SI convention; `xigma_i/gui_adapter.py` converts to/from CGS at its own boundary.
...
## Physical constants (`gammaforge.io`)

`kascade.py`'s "Physical constants (SI)" block (`C_LIGHT`/`E_CHARGE`/
`HBAR`/`EPS0`/`SIGMA_T`/`MEC2_EV`/`MEC2_J`) comes from `gammaforge.io.units`
...
agreed with `gammaforge.io`'s canonical values to their quoted precision
...
- `gui/` (Python package `gammaforge.gui`) — the Tkinter GUI. Consumes
...
- `io/` (package `gammaforge.io`) — shared physical constants/pint
