...
The canonical choices below match what both ``xigma_i`` and ``kascade``
already agree on internally (see each engine's ``Config``/
``params_to_config``): a Gaussian width is canonically the RMS of the
*intensity*/density profile, a duration is canonically a time in seconds
(even though both engines store the corresponding config field as
``c * duration`` in metres -- see ``units.py``'s light-time context), and a
laser amplitude is canonically its peak value. Nothing here is arbitrary;
it was chosen to make ``to_canonical`` a no-op convention-wise for every
current consumer's own ``ModelSpec`` (e.g. ``xigma_i.params.spec.
XIGMA_SPEC``), so a mismatch shows up as loudly as possible if a model's
