...
def save_electron_beam(beam: GaussianElectronBeam, path: str) -> None:
    doc = {
        "electron_beam": {
            ...
            "bunch_charge_pC": beam._q_C * 1e12,
            "kinetic_energy_MeV": beam._KE_eV * 1e-6,
            "rel_energy_spread_rms": beam.rel_energy_spread_rms,
            "sigma_x_rms_um": beam._sx_m * 1e6,
            "sigma_y_rms_um": beam._sy_m * 1e6,
            "emit_geom_x_um": beam._ex_m * 1e6,
            "emit_geom_y_um": beam._ey_m * 1e6,
            "bunch_duration_rms_ps": beam._st_s * 1e12,
...
def save_laser(pulse: GaussianParaxialLaser, path: str) -> None:
    doc = {
        "laser": {
            "model": _LASER_MODEL,
            "version": _SPEC_VERSION,
            "pulse_energy_J": pulse._E_J,
            "wavelength_um": pulse._wl_m * 1e6,
            "waist_rms_x_um": pulse._wx_m * 1e6,
            "waist_rms_y_um": pulse._wy_m * 1e6,
            "duration_rms_fs": pulse._dur_s * 1e15,
            "focus_z_um": pulse._fz_m * 1e6,
            "propagation_direction": "-z",
        }
    }
    with open(path, "w") as fh:
