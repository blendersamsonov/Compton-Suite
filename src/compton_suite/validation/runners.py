...
``compton_io.bunch.sample_gaussian_bunch`` -- mirroring the same
"macrobunching" change made to ``compton_guide.app.py``'s ``on_start()``
...
xigma-i-direct, ``DEFAULT_N_MC`` for kascade/analytical -- see
scenarios.py's own comment on this split) and seeds it with ``seed``
(default ``DEFAULT_SEED``). Because ``sample_gaussian_bunch`` is
deterministic given (beam, n_particles, seed), xigma-i and xigma-i-direct
...
defaults to 60_000, xigma-i-direct to 20_000 -- so this isn't guaranteed,
