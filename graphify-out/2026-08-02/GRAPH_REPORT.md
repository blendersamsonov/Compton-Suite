...
## Corpus Check
- 74 files · ~81,299 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1111 nodes · 2262 edges · 78 communities (55 shown, 23 thin omitted)
...
- `test_model()` --calls--> `validate_results()`  [INFERRED]
  scripts/headless_test.py → src/compton_suite/io/photons.py
...
Cohesion: 0.07
Nodes (72): Shared comparison metric for the cross-model validation suite.  Promoted (moved, s, spec, spec_ref: 1D arrays of equal shape (same grid for both     spectra -- r, Linear-interpolate spec_src(s_src) onto s_ref, zero outside s_src's range., resample_to(), window_integrated_relative_error(), User-curated bank of additional cross-validation parameter sets.  Deliberately e, Cross-model validation suite -- entry point.  Runs kascade/xigma-i/delta/analyti, Tier 0 (config-only, no run needed) + Tier 1/2 (share one run of     each model) (+64 more)
...
Cohesion: 0.13
Nodes (19): ballistic_position_z0_reference(), Straight-line position at time offset ``t``, given a per-particle     reference, Photon-density envelope of a Gaussian laser pulse at an arbitrary         point, _bin_spatial(), _bin_temporal(), _push_and_sample_vectorized(), PushDiagnostics, Stage 0: macroparticle source and ballistic pusher.  Produces, for a bunch of ma (+11 more)
...
Cohesion: 0.18
Nodes (18): _array_module(), build_table(), build_table_streaming(), _cell_indices(), _deposit(), deposit_cic(), deposit_nearest(), gamma_bracket() (+10 more)

### Community 7 - "Laser Specifications"
Cohesion: 0.11
Nodes (22): _drift_gaussian_fit(), propagate(), Validate a :class:`GaussianElectronBeam`.      Raises ``ValueError`` on hard req, Draw macroparticles from ``beam``. Delegates to     :func:`sample_gaussian_canon, Analytically propagate a :class:`GaussianElectronBeam`'s Twiss tilt     through, Ballistically drift every macroparticle in ``bunch`` by a time     offset ``dt``, Yield a propagated :class:`Bunch` snapshot at each time in     ``t_grid`` (SI se, sample_gaussian_bunch() (+14 more)

### Community 8 - "Photon Results"
Cohesion: 0.14
Nodes (10): GaussianParaxialLaser, Quantity, Rayleigh range for the x waist: ``pi * w0^2 / lambda`` with         ``w0 = 2 * w, RMS intensity-profile width in x at absolute position ``z_m``         (SI metres, Angular frequency, ``2*pi*c/wavelength``., Peak power at the pulse's temporal center, at focus:         ``E / (sqrt(2*pi) *, On-axis, peak-in-time intensity at absolute position ``z_m`` (SI         metres), Normalized vector potential at absolute position ``z_m`` (SI         metres), fr (+2 more)

### Community 9 - "Project Documentation"
Cohesion: 0.12
Nodes (22): AngularRangeSpectrumResult, BinnedAngularSpectrum, BinnedSpatialDistribution, BinnedSpectrum, BinnedTemporalEnvelope, detect_device(), get_xp(), Auto-detect which backend to use: a real CUDA GPU via cupy if     available, els (+14 more)
...
Cohesion: 0.14
Nodes (12): Photons, What every model's ``run()`` must return (shape-compatibly).      Only ``model_n, Adapter, angle_integrated_spectrum(), estimate_spectrum_width(), estimate_yield(), ndarray, Fast, closed-form Compton-source physics: total yield, angle-integrated spectrum (+4 more)

### Community 15 - "propagate"
Cohesion: 0.17
Nodes (7): Grid4D, Uniform 4D grid over (gamma, theta_x, theta_y, a0)., Derive grid extents from the populated data range, plus a margin         express, A deposited H table plus everything Stage 2 needs to consume it     without re-d, Turn an a0_kind='shape' table into a spectrum-ready a0_kind='ahat'     table for, retarget_a0(), Table
...
Cohesion: 0.08
Nodes (24): fit_gaussian(), Fit a :class:`GaussianElectronBeam` from raw macroparticles: a full     covarian, Draw macroparticles from a :class:`GaussianElectronBeam` using     canonical var, sample_gaussian_canonical(), Test structured Gaussian fitting., Verify fitted parameters match input beam., Verify fitting works correctly with drifted beam., Verify sigma_gamma is correctly calculated from fit. (+16 more)
...
### Community 24 - "app.py"
Cohesion: 0.26
Nodes (9): _float_or_none(), _native(), Refresh electron and laser values derived from the current fields., Compute (wavelength_m, sigma0_l_m, sigma_par_L_m, pulse_energy_J,         focus_, Compile the current Electrons + Laser panel fields into the         analytic bea, Convert a raw field value from the GUI's own display unit     (``native_unit``,, Transverse rms beam size [m] from normalised emittance, beta function,     and L, sigma_from_emittance() (+1 more)

### Community 25 - "TabulatedEngine"
Cohesion: 0.12
Nodes (11): Any, Defensive duck-type check, run right after a model's ``run()``     returns., validate_results(), table.total_weight -- retarget_a0 preserves it exactly., (t_seconds, rate) bin-center arrays -- photon-emission rate vs         time. Non, (x_centers, y_centers, density) -- transverse areal density         [photons/cm^, dN/ds, angle-integrated over all emission solid angle --         spectrum_from_p, d2N/(ds dOmega) grid -- spectrum4d.calculate_angular_spectrum_4d         on this (+3 more)
...
Cohesion: 0.12
Nodes (16): CollisionParams dead-field pruning (Phase 1 completed), sigma0_x/sigma0_l wrinkle (not raw inputs), XIGMA_SPEC, parameter-framework-and-collision-params refactor, adapt_to_model, AmplitudeConvention enum, canonical representation, CollisionParams (+8 more)
...
Cohesion: 0.07
Nodes (47): tests/ — test_constants.py, test_conversions.py, test_bunch.py, test_laser.py, test_io_formats.py for compton_suite.io, Enum, specs/electron_beam_io_v0.1_full.md / _short.md — gaussian_6d_waist spec, specs/gaussian_paraxial_laser_io_v0.1.md / _short.md — gaussian_paraxial spec, adapters/kascade_adapter.py — params_to_config reads compton_suite.io.constants directly, _pq(), Shortcut to build a PhysicalQuantity -- the GUI wraps its own raw     floats dir, _pq() (+39 more)
...
### Community 34 - "Bunch"
Cohesion: 0.18
Nodes (13): PhotonMultiplicity, Output-side observable representations: the spectrum/angular-spectrum/ temporal-, SampledSpatialDistribution, SampledSpectrum, SampledTemporalEnvelope, Job, The single compiled-from-UI config object the GUI passes to     ``ModelAdapter.r, _bunch_to_kascade_electrons() (+5 more)

### Community 35 - "drift"
Cohesion: 0.21
Nodes (8): drift(), Ballistically propagate a bunch by a longitudinal distance ``L`` (SI     metres,, Test vacuum propagation., Verify x -> x + x' * L., Verify z, thx, thy, gamma unchanged by drift., Verify Twiss alpha emerges from waist + drift., Verify drift() analytically propagates bunch.gaussian_fit's alpha         in loc, TestDrift

### Community 36 - "ValueError"
Cohesion: 0.16
Nodes (13): External-format I/O for compton_suite.io's bunch/laser representations., load_elegant_ele(), Elegant / SDDS ``.ele`` file I/O for :class:`compton_suite.io.bunch.Bunch`.  Rel, Parse a 6-D electron-bunch ``.ele`` file in SDDS ASCII format.      Required col, load_electron_beam(), load_laser(), YAML I/O for the ``gaussian_6d_waist``/``gaussian_paraxial`` v0.1 formats define, save_electron_beam() (+5 more)
...
Cohesion: 0.20
Nodes (10): laser_overlap_time_window(), Per-particle time window ``[t0, t1]`` (normalised length units,     e.g. ``k0_la, _get_numba_kernel(), _normalise_bunch(), push_and_sample(), _push_and_sample_numba(), Convert a ``compton_suite.io.bunch.MacroBunch`` (SI, external/engine-     agnost, Ballistically push each macroparticle and emit one sample per particle.      Eve (+2 more)

### Community 39 - "spectrum4d.py"
Cohesion: 0.22
Nodes (8): calculate_angular_spectrum_4d(), _get_numba(), get_spectrum_kernel_4d_cpu(), CPU/numba equivalent of spectrum4d.py's GPU rawkernel (spectrum_kernel_4d), used, Lazily compiles and caches spectrum_kernel_4d_cpu (see module docstring)., Stage 2: `spectrum_kernel_4d`, the GPU kernel that turns a Stage 1 4D overlap ta, Host-side driver for spectrum_kernel_4d, the Stage-2 analogue of     Compton.cal, ValueError
...
Cohesion: 0.16
Nodes (11): Bunch, evaluate_fit_quality(), ndarray, Macroparticle electron bunch. SI units, flat arrays.      ``x``/``y``/``z`` are, Total number of physical electrons., Evaluate Gaussian fit quality with sampling-noise baseline.      Compares the re, Test fit quality evaluation metrics., Verify Gaussian data produces low KS excess. (+3 more)

### Community 46 - "OutputSpec"
Cohesion: 0.21
Nodes (11): check(), main(), Mirrors app.py's on_start(): the always-on analytical preview runs     alongside, test_model(), test_preview_alongside(), Build an OutputSpec from the GUI fields., Build the ``Job.extra`` dict for ``adapter`` from its own         ``model_params, InteractionParameters (+3 more)
...
Cohesion: 0.20
Nodes (14): a0 formula discrepancy (RESOLVED), CUDA OOM with large electron bunches, a0_shape (trajectory-averaged a0), push_and_sample (Stage 0), PushDiagnostics, retarget_a0, spectrum4d_cpu (numba fallback), spectrum_kernel_4d (GPU kernel) (+6 more)
...
- **What is the exact relationship between `Geometric Convention (interaction point at origin, e- along +z, focus at IP, alpha=0)` and `Laser Pulse I/O Specification v0.1 (companion spec referenced by name, exact path inferred)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `GaussianElectronBeam` connect `Electron Beam Model` to `IO Core + Validation`, `Bunch Sampling`, `drift`, `ValueError`, `Laser Specifications`, `TestFitQuality`, `GUI Model Selection`, `fit_gaussian`, `sample_gaussian_canonical`, `test_bunch_improvements.py`, `app.py`, `TabulatedEngine`, `units.py`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `GaussianParaxialLaser` connect `Photon Results` to `IO Core + Validation`, `Bunch Sampling`, `GUI App Layout`, `Laser Overlap`, `ValueError`, `test_laser.py`, `TestFitting`, `OutputSpec`, `GUI Model Selection`, `app.py`, `TabulatedEngine`, `units.py`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `core-simulation-api refactor (completed)` connect `core-simulation-api refactor (completed)` to `build_params (CGS CollisionParams)`, `PhysicalQuantity`, `Canonical Sampling Tests`, `units.py`?**
