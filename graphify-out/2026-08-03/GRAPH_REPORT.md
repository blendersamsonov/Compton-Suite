...
## Corpus Check
- 74 files · ~80,711 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1107 nodes · 2203 edges · 70 communities (47 shown, 23 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 179 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `be3c6c66`
...
Cohesion: 0.06
Nodes (82): check(), main(), xigma-i/delta only: model_params() must expose a 'chunk' key (the     "query VRA, Mirrors app.py's on_start(): the always-on analytical preview runs     alongside, test_chunk_override(), test_model(), test_preview_alongside(), InteractionParameters (+74 more)

### Community 1 - "Bunch Sampling"
Cohesion: 0.06
Nodes (26): add_field_grid(), ComptonGUIApp, _float_or_none(), _native(), Return (total_flux, collimated_flux, cmask_or_None) [ph/s].          cmask is on, Convert a raw field value from the GUI's own display unit     (``native_unit``,, Place (label, default, key) triples in an n_cols-wide grid inside a     coloured, Load a 6-D electron bunch from an SDDS ``.ele`` file and turn the         Electr (+18 more)
...
Cohesion: 0.13
Nodes (20): Bunch, propagate(), Macroparticle electron bunch. SI units, flat arrays.      ``x``/``y``/``z`` are, Total number of physical electrons., Draw macroparticles from ``beam``. Delegates to     :func:`sample_gaussian_canon, Ballistically drift every macroparticle in ``bunch`` by a time     offset ``dt``, Yield a propagated :class:`Bunch` snapshot at each time in     ``t_grid`` (SI se, sample_gaussian_bunch() (+12 more)
...
Cohesion: 0.10
Nodes (18): fit_gaussian(), _pq(), Validate a :class:`GaussianElectronBeam`.      Raises ``ValueError`` on hard req, Fit a :class:`GaussianElectronBeam` from raw macroparticles: a full     covarian, Shortcut to build a PhysicalQuantity., validate(), Cross-checks for bunch.py's Bunch / GaussianElectronBeam / sample_gaussian_bunch, test_derived_quantities_are_sane() (+10 more)
...
### Community 24 - "app.py"
Cohesion: 0.47
Nodes (4): main(), Compton-GUIde: Tkinter GUI for pluggable Compton-scattering physics models., GammaForge: unified package for inverse-Compton scattering simulation.  Subpacka, run_gui()

### Community 25 - "TabulatedEngine"
Cohesion: 0.29
Nodes (8): ~2π angular-spectrum residual (open), convergence testing (resolution/particle/quadrature scans), spectrum4d_cpu (numba fallback), spectrum_kernel_4d (GPU kernel), fit_gaussian, GaussianElectronBeam, sample_gaussian_bunch, detect_device()
...
Cohesion: 0.08
Nodes (38): Enum, _pq(), Shortcut to build a PhysicalQuantity -- the GUI wraps its own raw     floats dir, Electron-bunch representations.  * :class:`Bunch` -- raw, engine-agnostic macrop, Shared physical constants, pint unit registry, parameter-semantics vocabulary, a, The shared "interaction parameters" bundle: one canonical (laser, electrons) pai, Quantum recoil parameter ``q = 4 * gamma * hbar * omega / (m_e * c^2)``.      Di, recoil_parameter() (+30 more)

### Community 31 - "api.py"
Cohesion: 0.11
Nodes (18): Protocol, Read the 0D slice every adapter is required to emit., What every model's ``run()`` returns.      ``photon_slices`` always includes a 0, Results, total_yield(), Adapter, Fast closed-form model: total yield, angle-integrated spectrum, and     an estim, discover_models() (+10 more)
...
Cohesion: 0.29
Nodes (6): Cross-checks for the width/time/amplitude convention conversions and the light-t, test_amplitude_round_trip(), test_duration_to_length_context_applies(), test_fwhm_to_sigma_conversion_applies(), test_time_round_trip(), test_width_round_trip()

### Community 46 - "OutputSpec"
Cohesion: 0.40
Nodes (4): Any, Output-side observable representations: the phase-space-slice contract every mod, Defensive sanity check, run right after a model's ``run()``     returns.      Re, validate_results()

### Community 48 - "build_params (CGS CollisionParams)"
Cohesion: 0.29
Nodes (8): CUDA OOM with large electron bunches, a0_shape (trajectory-averaged a0), push_and_sample (Stage 0), PushDiagnostics, retarget_a0, TabulatedEngine, MacroBunch, sample_gaussian_canonical
...
Cohesion: 0.09
Nodes (22): load_laser(), save_laser(), GaussianParaxialLaser, Photon count in the pulse: N_L = pulse_energy / (hbar*omega0). A         pure co, Validate a :class:`GaussianParaxialLaser`.      Raises ``ValueError`` on hard re, Only a paraxial Gaussian laser pulse *for now*.      Every field is a :class:`Ph, validate(), PhysicalQuantity (+14 more)
...
- **Why does `GaussianElectronBeam` connect `ValueError` to `IO Core + Validation`, `Bunch Sampling`, `drift`, `Laser Specifications`, `TestFitQuality`, `GUI Model Selection`, `fit_gaussian`, `sample_gaussian_canonical`, `test_bunch_improvements.py`, `test_laser.py`, `units.py`, `api.py`?**
