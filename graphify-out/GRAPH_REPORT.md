...
## Corpus Check
- 74 files · ~83,760 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1140 nodes · 2283 edges · 79 communities (52 shown, 27 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 184 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `af618340`
...
Cohesion: 0.10
Nodes (50): Shared comparison metric for the cross-model validation suite.  Promoted (moved, s, spec, spec_ref: 1D arrays of equal shape (same grid for both     spectra -- r, Linear-interpolate spec_src(s_src) onto s_ref, zero outside s_src's range., resample_to(), window_integrated_relative_error(), Cross-model validation suite -- entry point.  Runs kascade/xigma-i/delta/analyti, Tier 0 (config-only, no run needed) + Tier 1/2 (share one run of     each model), run() (+42 more)

### Community 1 - "Bunch Sampling"
Cohesion: 0.06
Nodes (28): add_field_grid(), ComptonGUIApp, _float_or_none(), main(), _native(), Refresh electron and laser values derived from the current fields., Compute (wavelength_m, sigma0_l_m, sigma_par_L_m, pulse_energy_J,         focus_, Build the ``Job.extra`` dict for ``adapter`` from its own         ``model_params (+20 more)

### Community 2 - "GUI App Layout"
Cohesion: 0.16
Nodes (24): KASCADE: Sequential multi-photon inverse-Compton Monte Carlo engine., beta_of(), build_lambda_grid(), Config, cos_collision(), doppler_D(), _eps_L(), kn_sigma_ratio() (+16 more)
...
Cohesion: 0.08
Nodes (32): PhasespaceSlice, Density over <=4 axes of the 6D photon phase space (time, space,     angle, ener, detect_device(), get_xp(), Auto-detect which backend to use: a real CUDA GPU via cupy if     available, els, Array module for ``device`` ('gpu' -> cupy, 'cpu' -> numpy) -- not a     unit co, Bring an array back to host (numpy) if it's on-device (cupy);     a no-op for CP, to_host() (+24 more)
...
Cohesion: 0.14
Nodes (16): propagate(), Draw macroparticles from ``beam``. Delegates to     :func:`sample_gaussian_canon, Ballistically drift every macroparticle in ``bunch`` by a time     offset ``dt``, Yield a propagated :class:`Bunch` snapshot at each time in     ``t_grid`` (SI se, sample_gaussian_bunch(), stream(), Cross-checks for bunch.py's Bunch / GaussianElectronBeam / sample_gaussian_bunch, test_derived_quantities_are_sane() (+8 more)
...
Cohesion: 0.14
Nodes (14): Compile the current Electrons + Laser panel fields into the         analytic bea, Save the current Electrons-panel fields as a         ``gaussian_6d_waist`` YAML, Save the current Laser-panel fields as a ``gaussian_paraxial``         YAML conf, Transverse rms beam size [m] from normalised emittance, beta function,     and L, sigma_from_emittance(), load_electron_beam(), load_laser(), save_electron_beam() (+6 more)
...
Cohesion: 0.21
Nodes (15): check(), main(), xigma-i/delta only: model_params() must expose a 'chunk' key (the     "query VRA, Mirrors app.py's on_start(): the always-on analytical preview runs     alongside, test_chunk_override(), test_model(), test_preview_alongside(), Build an OutputSpec from the GUI fields. (+7 more)
...
Cohesion: 0.06
Nodes (34): drift(), _drift_gaussian_fit(), fit_gaussian(), Fit a :class:`GaussianElectronBeam` from raw macroparticles: a full     covarian, Draw macroparticles from a :class:`GaussianElectronBeam` using     canonical var, Analytically propagate a :class:`GaussianElectronBeam`'s Twiss tilt     through, Ballistically propagate a bunch by a longitudinal distance ``L`` (SI     metres,, sample_gaussian_canonical() (+26 more)

### Community 19 - "test_bunch_improvements.py"
Cohesion: 0.12
Nodes (16): mildly_relativistic_beam(), _pq(), Tests for the improved electron bunch sampling, fitting, and evaluation.  This m, Build a PhysicalQuantity for test fixtures., Random number generator for reproducible tests., End-to-end integration tests., Verify sample_gaussian_bunch delegates to canonical sampling., Ultra-relativistic beam (gamma >> 1) for testing.      Physical parameters: 500 (+8 more)
...
Cohesion: 0.18
Nodes (13): _convert(), convert_amplitude(), convert_time(), convert_width(), fwhm_to_sigma_intensity(), Raised when a convention has no registered conversion in its family., UnknownConversionError, Cross-checks for the width/time/amplitude convention conversions and the light-t (+5 more)
...
### Community 24 - "app.py"
Cohesion: 0.24
Nodes (11): Like run_xigma, but always computes fresh and returns     ``(Photons, XigmaAdapt, Like run_delta, but always computes fresh and returns     ``(Photons, DirectAdap, run_delta_live(), run_xigma_live(), collimated_fraction(), Tier 3: angular shape canary (xigma-i vs delta), reported not gated.  xigma-i an, ``adapter`` is the live XigmaAdapter/DirectAdapter instance that     produced ``, Always returns True -- canary tier, reports findings, does not gate     the suit (+3 more)
...
Cohesion: 0.09
Nodes (34): Enum, _pq(), Shortcut to build a PhysicalQuantity -- the GUI wraps its own raw     floats dir, _pq(), Electron-bunch representations.  * :class:`Bunch` -- raw, engine-agnostic macrop, Shortcut to build a PhysicalQuantity., Shared physical constants, pint unit registry, parameter-semantics vocabulary, a, The shared "interaction parameters" bundle: one canonical (laser, electrons) pai (+26 more)

### Community 31 - "api.py"
Cohesion: 0.09
Nodes (24): Protocol, AngularRangeSpectrumResult, make_slice(), ndarray, Output-side observable representations: the phase-space-slice contract every mod, Read the 0D slice every adapter is required to emit., Histogram per-macroparticle samples (with a uniform weight) into a     :class:`P, On-demand angle-integrated spectrum restricted to an arbitrary     ``(theta_x_ra (+16 more)
...
Cohesion: 0.08
Nodes (12): GaussianElectronBeam, Quantity, The ``gaussian_6d_waist`` v0.1 I/O contract.      A 6D factorized Gaussian defin, Bunch population (charge / e). A pure count has no unit to be         agnostic a, Absolute RMS dispersion for longitudinal momentum (normalized to mc)., Correlation coefficient ρ_zγ = chirp_h · σ_z / σ_γ., Correlation coefficient ρ_xγ = D_x · σ_γ / σ_x., Correlation coefficient ρ_yγ = D_y · σ_γ / σ_y. (+4 more)
...
Cohesion: 0.13
Nodes (13): Bunch, Macroparticle electron bunch. SI units, flat arrays.      ``x``/``y``/``z`` are, Total number of physical electrons., load_elegant_ele(), Elegant / SDDS ``.ele`` file I/O for :class:`gammaforge.io.bunch.Bunch`.  Reloca, Write a :class:`Bunch` in SDDS ASCII ``.ele`` format.      Mirrors :func:`load_e, Parse a 6-D electron-bunch ``.ele`` file in SDDS ASCII format.      Required col, save_elegant_ele() (+5 more)

### Community 46 - "OutputSpec"
Cohesion: 0.14
Nodes (18): available_ram_bytes(), available_vram_bytes(), Free GPU memory in bytes via cupy, or None if no GPU/cupy usable --     the one, Available system RAM in bytes (POSIX sysconf), or None if the query     isn't su, estimate_chunk_size(), Auto-sized ``chunk`` for ``push_and_sample``, from currently free     memory on, angle_integrated_spectrum(), direct_binning_spectrum() (+10 more)

### Community 47 - "PhysicalQuantity"
Cohesion: 0.24
Nodes (11): _gamma_weight(), Coverage for spectrum_from_particles.angle_integrated_spectrum's chunk= streamin, Regression test for the reported bug: XigmaAdapter OOM'd allocating     a 5,000,, A chunk sized too large to fit in memory (but < len(s), so the     chunked/retry, Regression test for the follow-on report: 5,000,000 particles x     1,024 energy, test_cupy_auto_chunk_avoids_oom_at_reported_scale(), test_cupy_chunked_matches_numpy_unchunked(), test_cupy_oom_retry_halves_chunk_and_succeeds() (+3 more)
...
Cohesion: 0.29
Nodes (8): laser_axis(), laser_density(), _R_sf(), Laser transverse RMS (photon-density) width -- round-beam collapse     of the la, Rayleigh range for the round-beam-collapsed ``_sigma0_l``., Laser propagation unit vector n_L = (sin phi, 0, -cos phi).      phi = crossing_, Gaussian laser photon density propagating along n_L, focus at delta.      Reduce, _sigma0_l()

### Community 50 - "yaml_spec.py"
Cohesion: 0.29
Nodes (7): invert_lambda(), kn_over_thomson(), ndarray, Klein--Nishina / Thomson differential-cross-section ratio R_KN in [0,1].      ``, Sample (thx, thy) from L_theta (classical) or L_theta * R_KN (quantum).      Pro, Solve Lambda(tau_emit) = threshold row-by-row (Lambda monotonic in tau)., sample_emission_angle()

### Community 52 - "TestEdgeCases"
Cohesion: 0.47
Nodes (5): build_kascade_config(), check_interaction_identity(), Tier 0: wiring/units sanity.  Only kascade has a standalone ``Config`` object. x, kascade's Config.interaction must be the exact same     InteractionParameters ob, run()
...
Cohesion: 0.15
Nodes (11): GaussianParaxialLaser, Photon count in the pulse: N_L = pulse_energy / (hbar*omega0). A         pure co, Validate a :class:`GaussianParaxialLaser`.      Raises ``ValueError`` on hard re, Only a paraxial Gaussian laser pulse *for now*.      Every field is a :class:`Ph, validate(), Engine on the tabulated-energy pipeline (particles.py/deposition.py/ spectrum4d., Cross-checks for laser.py's GaussianParaxialLaser.  No cupy/GPU/tkinter needed., test_defocused_interaction_intensity_is_lower() (+3 more)
...
### Community 59 - "spectrum_kernel_4d (GPU kernel)"
Cohesion: 0.67
...
- **Why does `XigmaAdapter` connect `fit_gaussian` to `build_params (CGS CollisionParams)`, `Bunch`, `Canonical Sampling Tests`?**
...
- **Why does `GaussianParaxialLaser` connect `test_laser.py` to `IO Core + Validation`, `Bunch Sampling`, `GUI App Layout`, `Laser Overlap`, `test_laser.py`, `Electron Beam Model`, `Project Documentation`, `Canonical Sampling Tests`, `GUI Model Selection`, `ballistic_position_z0_reference`, `units.py`, `api.py`?**
