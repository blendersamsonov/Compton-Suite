...
| `src/gammaforge/gui/` | `gammaforge.gui` | Tkinter desktop GUI (`app.py`). Thin consumer of `io/` and `models/api.py` — no physics computation, only rendering, field parsing, and compiling a `Job` to hand to whichever adapter is selected. |
...
- **~~CUDA OOM with large electron bunches in xigma_i/delta~~ (resolved)**: `particles.push_and_sample(..., chunk=...)` now chunks the O(n_particles * n_steps) trajectory-integration step over particle slices, concatenating outputs (exact, since each particle's trajectory is independent) -- see `particles._integrate_trajectory_core`/`estimate_chunk_size`. `estimate_chunk_size(n_particles, n_steps, backend)` auto-sizes `chunk` from `gammaforge.misc.available_vram_bytes()` (capped at 70%) on `backend='cupy'`; a static per-(particle*step) byte estimate calibrated on a GTX 1660 Ti backs this, with an in-loop OOM-retry (halve remaining chunk size, bounded) as a safety net against the estimate being off. Both `XigmaAdapter`/`DirectAdapter` expose `chunk` via `model_params()` (`0` = auto). `deposition.build_table_streaming` remains a separate, unused utility (chunks bunch *sampling*, not the trajectory integration) -- not wired into either adapter, out of scope here.
  ...
  Stage 0's chunking above did **not** cover `TabulatedEngine.spectrum()` (`XigmaAdapter`'s dN/ds output) -- `spectrum_from_particles.angle_integrated_spectrum` broadcasts `gamma`/`weight` (length n_particles, up to several million) against the energy grid `s` (a few hundred points) into one `(n_particles, len(s))` float64 array with no chunking of its own, e.g. 10.24 GB for 5,000,000 particles x 256 energy points -- confirmed as a real user-reported OOM (`cupy.cuda.memory.OutOfMemoryError` inside `angle_integrated_spectrum`, not `push_and_sample`). Fixed the same way: `angle_integrated_spectrum(..., chunk=None)` now auto-sizes a chunk over the (small) `s` axis from free VRAM via `_estimate_s_chunk`, with the same halve-and-retry OOM safety net as `push_and_sample`. `TabulatedEngine.spectrum()` needs no change -- it already forwards `backend=self.backend` and picks up the new default. See `tests/test_angle_integrated_spectrum_chunking.py`.

  Separately (same bug report): `DirectAdapter` (`delta`) computed its Stage 2 (`angle_integrated_spectrum`/`direct_binning_spectrum`, the angular-spectrum grid loop) on host numpy arrays unconditionally, never passing `backend='cupy'` even when `device_preference='gpu'` resolved to an actual GPU -- so `delta` silently ran its most expensive step (an `n_theta_grid**2`-iteration loop, each a full-array histogram) on CPU regardless of device selection, which is what made it "use CPU even with GPU selected" and hang at large `n_particles`. Fixed in `DirectAdapter.run()`: both calls now pass `backend=push_backend` and use the on-device (not host-converted) `gamma`/`tx`/`ty`/`w`/`a0` arrays, converting each result back to host at the point of storage. `self.gamma`/`.theta_x`/etc. (cached for `spectrum_in_angular_range`'s on-demand CPU-only requery) are unaffected -- still populated from the host-converted arrays as before.

  Follow-on user report: the fix above only chunked `backend='cupy'` (mirroring `estimate_chunk_size`'s pre-existing "plain numpy is bounded by system RAM" assumption for `push_and_sample`) -- wrong at large enough problem sizes. 5,000,000 particles x 1,024 energy bins exhausted 128GB of RAM and crashed the machine on `backend='numpy'` (no GPU/CPU-device run), the exact same unchunked-broadcast shape, just on the host instead of the device. Fixed by adding `gammaforge.misc.available_ram_bytes()` (POSIX `os.sysconf('SC_AVPHYS_PAGES')`/`'SC_PAGESIZE'`) as the RAM-side analogue of `available_vram_bytes()`, and chunking `backend='numpy'` in both `spectrum_from_particles._estimate_s_chunk` and `particles.estimate_chunk_size` against it (only `backend='numba'` stays exempt -- its own per-particle compiled loop genuinely has no O(n*n_steps) blowup to chunk around). Host chunking uses a more conservative `safety_frac=0.5` (vs. cupy's `0.7`): an oversized host allocation can trigger the Linux OOM-killer or heavy swapping instead of a catchable exception, a much larger blast radius to get wrong than a contained `cupy.cuda.memory.OutOfMemoryError` -- the halve-and-retry loops now also catch `MemoryError` on the numpy path as a secondary safety net, but the proactive `available_ram_bytes()`-based estimate is the primary defense since overcommit means a too-large allocation isn't guaranteed to raise before the OOM-killer acts.

  Also from the same session: `spectrum_in_angular_range(theta_x_range, theta_y_range, n_energy=...)`'s `n_energy` (default 96 for `XigmaAdapter`, 65 for `DirectAdapter`) was never threaded from the GUI's "Energy bins" field (`n_energy_bins`) -- `gui/app.py`'s `_render_spectrum`/`_photon_fluxes` called `spectrum_in_angular_range` with no `n_energy` argument, so the "collimated" spectrum curve and the collimated-flux stat always used the adapter's hardcoded default regardless of what the user set, while the "all (4*pi)" curve (from the `AXIS_ENERGY`-only slice, correctly sized from `n_energy_bins` via `_get_output_spec`) did respond -- reported as "changing energy_bins doesn't change the [collimated] plot". Fixed by passing `n_energy=n_energy_bins` explicitly at both call sites.

  **Follow-on regression (same day): `_estimate_s_chunk`'s memory-fraction sizing was itself the bug.** Sizing the chunk as "up to `safety_frac` of whatever memory happens to be free" grows the chunk in direct proportion to installed RAM -- on a well-provisioned (128GB) machine this computed a chunk in the hundreds, using ~100GB for a computation a clean, warmed-up, repeated benchmark (n_particles=5,000,000, n_s=1024, 3 reps, memory pool reset between trials) showed runs equally fast (within noise) at chunk=4/8/16 as at chunk=1024: 7.29s/7.25s/7.20s vs. chunk=1's 9.99s -- the batching only exists to amortise cupy/numpy per-op dispatch overhead, and that's fully captured by a small chunk; bigger buys nothing. Fixed by adding a hard `_MAX_S_CHUNK = 32` ceiling: the memory budget can only shrink the chunk *below* this on a constrained machine, never grow it past this on a generous one. See `tests/test_angle_integrated_spectrum_chunking.py::test_estimate_s_chunk_capped_regardless_of_free_memory`.

  **Follow-on: the new Angle-Resolved Spectra panel (see GUI section below) ignored the collimation-angle fields.** It originally always read the joint `(AXIS_ENERGY, AXIS_THETA_X, AXIS_THETA_Y)` slice from `res.photon_slices` -- a coarse, fixed generous window (`_theta_grid`'s `half_window = 3.0/gamma0`) unrelated to `theta_x_col_mrad`/`theta_y_col_mrad`. Fixed by exposing the joint `(theta_x, theta_y, energy)` grid both adapters' `spectrum_in_angular_range()` already build internally before collapsing it to their own 1D angle-integrated spectrum -- `AngularRangeSpectrumResult.angle_energy_grid` (new field, a `PhasespaceSlice`, `None` only if unpopulated) costs nothing extra to compute. `_render_angle_resolved_spectra` now calls `spectrum_in_angular_range((-tx, tx), (-ty, ty))` and uses that grid when a collimation window is set and the model supports it (xigma-i, delta), falling back to the old wide-window joint slice otherwise (no collimation set, or kascade/analytical which have no `spectrum_in_angular_range`) -- same fallback pattern `_render_spectrum`'s "collimated" curve already uses.

  **Follow-on regression: `spectrum_in_angular_range`'s `n_energy=n_energy_bins` threading (two entries up) made an already-slow path catastrophic, and it fired far more often than necessary.** Three separate problems, found together via a user-reported hard OOM-kill at 2M particles / 2048 energy bins with no Python traceback:

  1. `theta_x_col_mrad`/`theta_y_col_mrad` are wired via `StringVar.trace_add("write", ...)`, which fires on every keystroke (not Enter/blur) -- and the SAME trace loop was registered twice (`gui/app.py`'s `_wire_live_updates`, a pre-existing duplicate-registration bug), so `_update_outputs()` fired twice per keystroke. Fixed: single registration, now debounced through `_on_collimation_field_changed` (`self.after(200, self._update_outputs)`, cancel-and-reschedule) -- confirmed 4 rapid writes collapse into exactly 1 `_update_outputs()` call.
  2. Each `_update_outputs()` independently called `spectrum_in_angular_range()` three times (once each in `_photon_fluxes`, `_render_spectrum`, `_render_angle_resolved_spectra`) with identical arguments. Fixed: computed once in `_update_outputs` and threaded through as `rng_result` to all three (and to `_render_plots`) instead of each recomputing it.
  3. The real culprit: `calculate_angular_spectrum_4d`'s cost is ~linear in `n_energy` and steep -- measured `n_energy=64` at 27s on CPU/numba (so even the ORIGINAL 96 default, pre-dating any of this session's changes, was already ~40s -- a pre-existing perf ceiling in the kernel itself, not something this session introduced), scaling to `n_energy=2048` at 14+ minutes (didn't finish; killed for this investigation) -- confirmed via direct RSS sampling to NOT be a memory leak (flat ~4.2GB the whole time, scales linearly with `n_particles` only), so the "OOM" was really the system becoming unresponsive under sustained 500%+ CPU, not exhausted memory. GPU/cupy is ~15x faster per unit but still linear (`n_energy=2048` measured at 50s) -- neither backend can absorb an unbounded, GUI-automatic (not "Calculate"-initiated) request. Fixed with `_MAX_LIVE_N_ENERGY_CPU=96`/`_MAX_LIVE_N_ENERGY_GPU=256`, applied inside both adapters' `spectrum_in_angular_range()` regardless of what `n_energy` the caller passes -- the GUI's "Energy bins" field still controls the FULL resolution for the deliberate, one-shot "Calculate" spectrum (`TabulatedEngine.spectrum()`/`angle_integrated_spectrum`, chunked, no per-call ceiling), just not this automatic live-update path. See `tests/test_spectrum_in_angular_range_cap.py`.

  **Follow-on: `models/analytical.py` had the identical unchunked-broadcast bug in a completely separate, previously-unaudited copy.** User report: `angle_integrated_spectrum` there (a deliberately independent copy of `xigma_i.spectrum_from_particles.angle_integrated_spectrum`'s formula, per that module's own docstring -- "kept independent... since xigma_i is production code... this fast-preview model doesn't need") hit a 76.3 GiB numpy allocation at 5,000,000 particles x 2048 energy bins, in the always-on analytical preview that runs alongside whichever main model is selected on every Calculate click, using that model's SAME "Energy bins" field. **Root cause, not just a missed chunking fix**: `estimate_yield`/`estimate_spectrum_width` are genuinely closed-form (only `beam.gamma0`/`.sigma_gamma`/etc., never a macroparticle) but `angle_integrated_spectrum` wasn't -- it took `electrons.gamma`/`electrons.weight` and Monte-Carlo-summed over the REAL sampled macroparticles, the only place this supposedly "closed form, no per-particle Monte Carlo" model (see its own module docstring) actually touched one, and the only reason its cost scaled with `n_particles` at all. Fixed by replacing the macroparticle sum with a fixed-size (`n_quad=401`, default) numerical quadrature over the beam's own Gaussian energy distribution (`gamma0`, `sigma_gamma`) -- same per-particle Compton-edge formula, now integrated against the *known* analytic distribution instead of Monte-Carlo-summed over samples of it. New signature: `angle_integrated_spectrum(gamma0, sigma_gamma, N_e, s, n_quad=401)`, no array-shaped particle-count argument at all. `Adapter.run()` no longer reads `job.interaction.electrons` for anything except `.gaussian_fit` (never the macroparticle arrays themselves). This eliminates the whole bug class outright rather than bounding it: no chunking machinery needed any more (removed), cost is now `O(n_quad * len(s))`, completely independent of `n_particles` -- verified at the exact reported scale (5,000,000 particles, 2048 energy bins) through the real `Adapter.run()` path: 0.018s (was a hard crash). See `tests/test_analytical.py`'s `test_angle_integrated_spectrum_matches_monte_carlo_reference` (validates the quadrature against an independent 2,000,000-sample MC reference) and `test_angle_integrated_spectrum_fast_at_reported_scale`.
